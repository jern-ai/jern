module Iron.Tests.SessionTests
...
[<Fact>]
let ``performing iron/llm-call reaches the bridge and returns its reply`` () =
    let session = newSession (mockBridge "mock says hi")
    let result =
        run session
            """(response-text (perform iron/llm-call (list :messages (vector))))"""
    match result with
...
let ``bridge receives the request the agent authored`` () =
    ...
    run session """(perform iron/llm-call (list :model "claude-opus-5" :max_tokens 7))"""
    |> ignore
...
let ``bridge errors surface as kernel errors`` () =
    ...
    let session = newSession bridge
    match Session.runSource session "test.ikr" "(perform iron/llm-call (list))" with
...
let ``agent code cannot reach the host bridge directly`` () =
    ...
    match Session.runSource session "test.ikr" "(iron/host-llm-call (list))" with
    | Choice1Of2 error ->
