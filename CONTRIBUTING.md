...
iron is Apache-2.0 (see [LICENSE](LICENSE)). Contributions are welcome under
the same license.
...
- `dotnet test Iron.slnx` must be green, including the agents' own suites
  (`iron test agents/default`, `iron test agents/docs`).
- Changes to agent behavior fail fixture replay by design; re-record with
  `iron test --record` (or the generator tests) and commit the new fixtures
  with the change.
