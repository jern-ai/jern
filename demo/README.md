...
**Act 1 — use it** *(needs `ANTHROPIC_API_KEY`)*. `jern run` fixes the failing
...
`.jern/trace-*.jsonl` has one event per effect — llm calls, tool calls, and
**every policy decision**. That trace is also the raw material for fixtures.

**Act 2 — read it.** `jern eject` drops the agent into the workspace.
...
**Act 3 — test it** *(no key)*. `jern test agents/default` — three tests, green,
...
edit is one line that switches it on unconditionally. No recompile. Rerun `jern test`: **it fails**, pointing at exactly where the
...
`jern test --record` blesses the change, and rerunning `jern run --agent`
