...
///
/// Configuration is JSON, merged from `~/.config/iron/config.json` then
/// `<workspace>/iron.json` (workspace wins per key):
///
///   {
    ...
    let load (workspaceRoot: string) : Result<Config, string> =
        let files =
            [ Path.Combine(Environment.GetFolderPath Environment.SpecialFolder.UserProfile,
                           ".config", "iron", "config.json")
              Path.Combine(workspaceRoot, "iron.json") ]
        try
            Ok(files |> List.fold applyFile defaultConfig)
        with ex ->
            Error("bad iron config: " + ex.Message)
