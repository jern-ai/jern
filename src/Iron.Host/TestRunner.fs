...
/// `iron test` — run an agent package's test suite.
///
...
/// workspace, with `iron/llm-call` answered by the fixture bridge: replay by
...
module TestRunner =
    ...
    let private runOne (agentDir: string) (mode: Fixtures.Mode) (file: string)
        ...
        let workspace = Path.Combine(Path.GetTempPath(), "iron-test-" + Guid.NewGuid().ToString("N"))
