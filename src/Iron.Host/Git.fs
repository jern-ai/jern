...
/// `iron undo`. Every iron-authored commit carries the author
/// `iron <iron@localhost>`, which is what makes undo safe: it only ever
/// pops a commit iron itself made.
module Git =

    let author = "iron <iron@localhost>"
    let private authorEmail = "iron@localhost"
    ...
    let private run (root: string) (args: string list) : Result<string, string> =
        ...
        for a in [ "-c"; "user.name=iron"; "-c"; "user.email=" + authorEmail ] @ args do
    ...
    /// The subject of HEAD if iron authored it.
    ...
    /// Undo the last iron-authored commit (hard reset by one). Refuses when
    /// HEAD is not iron's, so it can never destroy the user's own work.
    let undoLast (root: string) : Result<string, string> =
        if not (isRepo root) then Error "not a git repository"
            ...
            | None -> Error "HEAD is not an iron commit; nothing to undo"
            | Some subject ->
