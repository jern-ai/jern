...
/// Host tool implementations, dispatched by name from the iron/tool-call
...
module Tools =
        ...
        | Ok command ->
            ...
            if OperatingSystem.IsMacOS() && File.Exists sandboxExec then
                ...
                if not warnedNoSandbox then
                    ...
                    eprintfn "iron: no OS sandbox available for shell commands; approval is the only gate"
    ...
    /// Dispatch an iron/tool-call payload `(:name "…" :input (…))`.
