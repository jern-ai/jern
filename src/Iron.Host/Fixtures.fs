...
/// Deterministic LLM fixtures — the machinery behind `iron test`.
///
        ...
        member _.Use(relativePath: string) : ThrowsError<unit> =
            ...
            | Replay ->
                if not (File.Exists path) then
                    Choice1Of2 (Default(sprintf "fixture '%s' not found — run `iron test --record` to create it" relativePath))
                else
                    try
